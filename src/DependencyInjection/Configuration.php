            ...
            ->children()
                ->arrayNode('index_service')
                    ...
                    ->children()
                        ...
                        ->arrayNode('reindex')
                            ->addDefaultsIfNotSet()
                            ->info('Settings for the asynchronous _reindex (task polling) used during deployment reindex.')
                            ->children()
                                ->integerNode('poll_interval_seconds')
                                    ->defaultValue(5)
                                    ->min(1)
                                    ->info('How often to poll the OpenSearch / Elasticsearch tasks API while a reindex runs.')
                                ->end()
                                ->integerNode('max_wait_seconds')
                                    ->defaultValue(7200)
                                    ->min(1)
                                    ->info('Maximum total time to wait for a reindex task to complete before aborting and cleaning up.')
                                ->end()
                            ->end()
                        ->end()
