...
final class SnapshotImporter implements SnapshotImporterInterface
{
    ...
    public function import(
        ...
        $skipped = [];
        $plan = $this->plan($manifest->getIndices(), $report, $skipped);
    ...
    {
        if ($options->isForce() || $report->isCompatible()) {
            return;
        }
    ...
    private function plan(array $entries, CompatibilityReport $report, array &$skipped): array
    {
        ...
        foreach ($entries as $entry) {
            ...
            if ($target === null) {
                $skipped[$entry->getShortName()] = 'no local counterpart '
            ...
            if ($classStatus === ClassCompatibilityStatus::INCOMPATIBLE) {
                $skipped[$entry->getShortName()] = 'class mapping incompatible, skipped because of --force';

                continue;
            }
            if ($classStatus === ClassCompatibilityStatus::UNVERIFIED) {
                $skipped[$entry->getShortName()] =
                    'no class mapping checksum in the manifest, skipped because of --force';
    ...
    private function downloadAndVerify(
        ...
        $file = $entry->getFile();
        if (preg_match('/^[A-Za-z0-9._-]+$/D', $file) !== 1 || in_array($file, ['.', '..'], true)) {
            throw new SnapshotImportException(sprintf('Invalid file name "%s" in manifest', $entry->getFile()));
        }
        ...
        $expectedFile = $entry->getShortName() . '.ndjson.gz';
        if ($entry->getFile() !== $expectedFile) {
        ...
        try {
            ...
            $storage->readFileToLocal($name, $entry->getFile(), $local);
            $this->documentFileReader->verifySize($local, $entry->getBytes());
            $this->documentFileReader->verifyHash($local, $entry->getSha256());
    ...
    private function replay(ManifestIndex $entry, IndexTarget $target, string $local): ImportedIndex
    {
        ...
        // the per-class reindex (ClassDefinitionReindexService) would then never self-heal.
        ...
        $this->searchIndexService->refreshIndex($target->getAliasName());
        $actual = $this->searchIndexService->getCount(new Search(), $target->getAliasName());
        ...
        $this->assertReplayComplete($target, $actual, $entry->getDocumentCount());
        $this->stampClassMapping($target, $classMappingChecksum);

        return new ImportedIndex($target->getShortName(), $target->getAliasName(), $entry->getDocumentCount(), $actual);
    }
